...
public abstract class IntrospectedTable {
    ...
    /** unqiue index except primary. */
    protected Map<String,List<IntrospectedColumn>> uniqueIndexes ;
    
    /** all index except primary. */
    protected Map<String,List<IntrospectedColumn>> otherIndexes ;
    ...
    public IntrospectedTable(TargetRuntime targetRuntime) {
        ...
        uniqueIndexes = new HashMap<String,List<IntrospectedColumn>>() ;
        otherIndexes = new HashMap<String,List<IntrospectedColumn>>() ;
    ...
    /**
     * Returns the Index Colums. If the generatePrimaryKeyClass()
     * method returns false, then these columns will be iterated as the
     * parameters of the selectByPrimaryKay and deleteByPrimaryKey methods
     * 
     * @return a List of ColumnDefinition objects for columns in the primary key
     */
    public Map<String,List<IntrospectedColumn>> getUniqueIndexes() {
     ...
     * @return a List of ColumnDefinition objects for columns in the primary key
     */
    public Map<String,List<IntrospectedColumn>> getOtherIndexes() {
        return otherIndexes;
    }
    ...
    /**
     * Checks for unique index.
     *
     * @return true, if successful
     */
    public boolean hasUniqueIndexs() {
    	return uniqueIndexes.size() > 0 ;
    }
     ...
     * @return true, if successful
     */
    public boolean hasOtherIndexs() {
    	return otherIndexes.size() > 0 ;
    }
    /**
    ...
    /**
     * Adds the index key column.
     *
     * @param columnName
     *            the column name
     */
    public void addUniqueIndexColumn(Map<String,List<String>> mapUniqueIndex) {
        for(String key : mapUniqueIndex.keySet()){
        	List<String> listColName = mapUniqueIndex.get(key) ;
        	List<IntrospectedColumn> listColums = new ArrayList<IntrospectedColumn>() ;
        	for(String colName : listColName){
        		Iterator<IntrospectedColumn> iter = baseColumns.iterator();
                while (iter.hasNext()) {
                    IntrospectedColumn introspectedColumn = iter.next();
                    if (introspectedColumn.getActualColumnName().equals(colName)) {
                    	listColums.add(introspectedColumn);
                    }
                }
        	}
        	if(listColums.size() > 0){
        		uniqueIndexes.put(key, listColums) ;
        	}
        }
    }
    
    public void addOtherIndexColumn(Map<String,List<String>> mapOtherIndex) {
        for(String key : mapOtherIndex.keySet()){
        	List<String> listColName = mapOtherIndex.get(key) ;
        	List<IntrospectedColumn> listColums = new ArrayList<IntrospectedColumn>() ;
        	for(String colName : listColName){
        		Iterator<IntrospectedColumn> iter = baseColumns.iterator();
                while (iter.hasNext()) {
        	...
        	if(listColums.size() > 0){
        		otherIndexes.put(key, listColums) ;
        	}
        }
    }
