...
public class DatabaseIntrospector {
	...
	public List<IntrospectedTable> introspectTables(TableConfiguration tc) throws SQLException {
		...
		Map<ActualTableName, Map<String, List<String>>> uniqueIndexes = getUniqueIndex(tc);
		Map<ActualTableName, Map<String, List<String>>> otherIndexes = getOtherIndex(tc);
		...
		List<IntrospectedTable> introspectedTables = calculateIntrospectedTables(tc, columns, uniqueIndexes, otherIndexes);
	...
	private Map<ActualTableName, Map<String, List<String>>> getUniqueIndex(TableConfiguration tc) throws SQLException {
		...
		Map<ActualTableName, Map<String, List<String>>> answer = new HashMap<ActualTableName, Map<String, List<String>>>();

		ResultSet rs = databaseMetaData.getIndexInfo(localCatalog, localSchema, localTableName, true, false);
		Map<String, List<String>> mapIndex;
		...
		while (rs.next()) {
			...
			mapIndex = answer.get(atn);
			if (mapIndex == null) {
				mapIndex = new HashMap<String, List<String>>();
				answer.put(atn, mapIndex);
			}
			...
			if (idxName.equals("PRIMARY")) {
				...
				idxName = idxName.substring(idxName.indexOf("IDX_")+4) ;
				String preIdx = idxName.substring(0, 1) ;
				String postIdx = idxName.substring(1) ;
				idxName = preIdx.toUpperCase() + postIdx.toLowerCase() ;
				listColName = mapIndex.get(idxName) ;
				if(listColName == null){
					listColName = new ArrayList<String>() ;
				}
			}
			
			if (!preIdxName.equals(idxName)) {
				mapIndex.put(idxName, listColName);
	...
	private Map<ActualTableName, Map<String, List<String>>> getOtherIndex(TableConfiguration tc) throws SQLException {
		...
		ResultSet rs = databaseMetaData.getIndexInfo(localCatalog, localSchema, localTableName, false, false);
		Map<String, List<String>> mapIndex;
		List<String> listColName;
		String preIdxName = "";
		while (rs.next()) {
			...
			String idxName = rs.getString("INDEX_NAME");
			String idxtype = rs.getString("NON_UNIQUE");  // 0 是唯一，1是非唯一
			
			if (idxName.equals("PRIMARY") || idxtype.equals("false")) {
				continue;
			}else{
				idxName = idxName.substring(idxName.indexOf("IDX_")+4) ;
				String preIdx = idxName.substring(0, 1) ;
	...
	private List<IntrospectedTable> calculateIntrospectedTables(TableConfiguration tc,
			Map<ActualTableName, List<IntrospectedColumn>> columns, 
			Map<ActualTableName, Map<String, List<String>>> uniqueIndexes,
			Map<ActualTableName, Map<String, List<String>>> otherIndexes) {
		boolean delimitIdentifiers = tc.isDelimitIdentifiers() || stringContainsSpace(tc.getCatalog())
				|| stringContainsSpace(tc.getSchema()) || stringContainsSpace(tc.getTableName());
		...
		for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
			...
			Map<String, List<String>> mapIndex = uniqueIndexes.get(atn) ;
			if (mapIndex != null){
				introspectedTable.addUniqueIndexColumn(mapIndex);
			}
			mapIndex = otherIndexes.get(atn) ;
			if (mapIndex != null){
				introspectedTable.addOtherIndexColumn(mapIndex);
			}
